...
# Read GPU data
gpu_df = pd.read_csv('gpu_perf_vec_openmp.csv')
gpu_sizes = gpu_df['Size']
gpu_flops = gpu_df['FLOP/s']

# Read CPU data
cpu_df = pd.read_csv('cpu_perf_vec_openmp.csv')
...
plt.ylabel('FLOP/s')
plt.title('Vecadd Solver Performance: GPU vs CPU (Best)')
plt.legend()
plt.grid(True, which="both", ls="--")
plt.tight_layout()
plt.savefig('vecadd_gpu_vs_cpu_best.png', dpi=300)
...
plt.ylabel('FLOP/s')
plt.title('CPU Scaling: Vecadd Solver Performance')
plt.xscale('log', base=2)
plt.legend(title='Matrix Dimension', loc='upper left', bbox_to_anchor=(1,1))
plt.grid(True, ls="--")
plt.tight_layout()
plt.savefig('vecadd_cpu_scaling.png', dpi=300)
...
plt.xlabel('Matrix Dimension')
plt.ylabel('FLOP/s')
plt.title('Vecadd Solver: GPU vs CPU (All Thread Counts)')
plt.legend()
plt.grid(True, which="both", ls="--")
plt.tight_layout()
plt.savefig('vecadd_gpu_vs_cpu_all.png', dpi=300)

print("Plots saved: vecadd_gpu_vs_cpu_best.png, vecadd_cpu_scaling.png, vecadd_gpu_vs_cpu_all.png")